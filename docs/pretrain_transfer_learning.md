...
1. **Mở băng một phần vision** (vài lớp cuối QA-ViT/ConvNeXt) — điều kiện cần để mọi mục tiêu vision/grounding có ý nghĩa.
2. **TWC Việt-hoá + OCR-denoise generative** — đưa error-tolerance vào cả encoder (TWC) lẫn decoder (denoise).
