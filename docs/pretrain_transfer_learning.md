...
2b. **Whole-word MLM masking** ✅ **ĐÃ TRIỂN KHAI** (nhánh `exp/pretrain-gen-all`, chỉ `data/collator.py`): mask TRỌN từ (mọi subword của một từ OCR cùng bị mask) thay vì per-subword → bỏ "nạng" copy nội-bộ-token, buộc khôi phục từ nhánh feature/ảnh (đúng TWA). Toggle `mlm_mask_mode` (arg `--mlm_mask_mode`, env `MLM_MASK_MODE`) = `wholeword`(mặc định)/`subword`. Đã verify 200 seed: 0 vi phạm partial-word. Kèm **diagnostic đo "nạng"** (`_diagnose_mlm_crutch` trong `training/pretrain.py`, chạy ở smoke): so MLM-acc FULL vs OCR-feature-ABLATED — drop nhỏ = còn nạng, drop lớn = grounding tốt. Chạy A/B: `MLM_MASK_MODE=wholeword` vs `subword`.
